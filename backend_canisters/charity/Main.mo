...
actor Charity {
  ...
  public shared query func getAllCharities(searchParams: ?Text) : async Result.Result<[Charity], Text> {
    let allCharities = Iter.toArray(charities.vals()); 
    switch(searchParams){
      case null{
        return #ok(allCharities);
      };
      case(?params){
        let filteredCharities = Array.filter<Charity>(allCharities, func (charity: Charity): Bool {
          Text.contains(charity.title, #text params)
        });

        switch(filteredCharities.size()){
          case 0{
            return #err("Charity Not Found");
          };
          case (num){
            return #ok(filteredCharities);
          };
        }
      }
    };
  ...
  public shared (msg) func updateCharity(charity_id : Text, charity : Charity) : async Result.Result<(), Text> {
    if (Principal.isAnonymous(msg.caller)) {
      return #err("Unauthorized");
    };
    let targetCharity = charities.get(charity_id);

    switch (targetCharity) {
      case null {
        return #err("Charity not found");
      };
      case (?c) {
  ...
  public shared (msg) func deleteCharity(charity_id : Text) : async Result.Result<?Charity, Text> {
    if (Principal.isAnonymous(msg.caller)) {
      return #err("Unauthorized");
    };
  ...
  public shared func seedCharity() : async Result.Result<Text, ()> {
    ...
    while(currentIdx < total_dummy_data) {
      let charityId = await UtilityProvider.getUUID();
      let charity = {
        id = charityId;
      ...
      charities.put(charityId, charity);
      currentIdx += 1;
    };
